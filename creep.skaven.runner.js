...
Creep.summonSkavenRunner = function(room, runners) {
  console.log('should we summon a runner?', room.controllerLevel, runners.length, room.energyAvailable);
  // Summon a Gutter Runner if we meet certain criteria
  if (room.controllerLevel >= 5 && runners.length === 0 && room.energyAvailable > 1000) {
    console.log('yes...')
